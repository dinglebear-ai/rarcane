...
test("release-please paths cover and transform every published version", () => {
  ...
  assert.equal(server.packages.find((entry) => entry.identifier === "@dinglebear/rarcane-mcp").version, next);
  assert.equal(publisher.distribution.npm, `@dinglebear/rarcane-mcp@${next}`);
