...
Test 6 custom MCP servers (unrust, rustscale, rustifi, rustify, apprise-mcp, rarcane) using mcporter with their external URLs, generate secure bearer tokens for all of them, and add them to the lab gateway at lab.example.internal with protected paths named after each repo.
...
**Follow-on tasks:**
- Configure Authelia access rules for `https://mcp.example.internal/unrust`, `/rustify`, `/rustifi`, `/rustscale`, `/apprise-mcp`, `/rarcane` if required for gateway probing
